...
class AskDetail:
    def POST(self):
        ...
        return {'question_data': question_data, 'reply_data': reply_data}
...
class AskList:
    def GET(self):
        ...
        if asker_urn == '':
            return web.badrequest()
        return list(Store.Question.fetch(askerUrn=asker_urn))

class AskCreate:
    def POST(self):
        request_input = web.input(askerUrn='', data={}, recipients='')
        asker_urn = request_input.askerUrn
        data = ast.literal_eval(request_input.data)
        recipients = request_input.recipients
        recipients = recipients.split(',')
        if asker_urn == '' or data == {} or recipients == []:
            return web.badrequest()
        ...
        for recipient in recipients:
            key = StoreUtils.ReplyPromise.createKey(targetMember=recipient, questionUrn=question_urn, askerUrn=asker_urn)
...
class RespondFetchQuestionsAskedToMe:
    def GET(self):
        ...
        if target_member == '':
            return web.badrequest()
        return list(Store.ReplyPromise.fetch(targetMember=target_member))
...
class RespondFetchQuestionDetail:
    def POST(self):
        ...
        if asker_urn == '' or question_urn == '':
            return web.badrequest()
        key = StoreUtils.Question.createKey(askerUrn=asker_urn, questionUrn=question_urn)
        return Store.Question.get(key=key)
...
class RespondReply:
    ...
    def POST(self):
        ...
        return Store.Reply.put(key=key, data=ast.literal_eval(request_input.data))
...
class MemberCreate:
    def POST(self):
        ...
        return Store.Member.put(key=phone_number, data=data) # true on success, false otherwise.
...
class MemberLogin:
    def POST(self):
        ...
        if phone_number == '':
            return web.badrequest()
        return Store.Member.get(key=phone_number)  # returns the memberUrn associated with the phone_number
...
class ContactList:
    def GET(self):
        ...
        if actor_urn == '':
            return web.badrequest()
        return list(Store.Contact.fetch(actorUrn=actor_urn))
...
class ContactAdd:
    def POST(self):
        ...
        return Store.Contact.put(key=key, data=data)
